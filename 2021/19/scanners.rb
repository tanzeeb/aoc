...
def match a, b, threshold
  ...
  orientations.product(directions).each do |(orientation,direction)|
    ...
    return beacons, translation if (a & beacons).size >= threshold
  end
...
until scanners.empty?
  ...
  matches, translation = match(beacons, scanner, [12,scanner.size].min)
...
puts positions.permutation(2).map {|a,b| a.zip(b).sum {|ai,bi| (ai-bi).abs} }.max
