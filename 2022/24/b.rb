...
Valley = Struct.new(:start) do
  ...
  def travel_time to, from, time
    queue = [[time, from]]
    visited = Set[[time, from]]
  ...
  def trip_time *destinations
    time = 0

    destinations.each_cons(2) do |(from,to)|
      time = travel_time to, from, time
    end
...
puts valley.trip_time(valley.entrance, valley.exit, valley.entrance, valley.exit)
